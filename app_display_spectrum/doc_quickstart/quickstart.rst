...
To setup up the system:
   ...
   #. Connect XA-SK-SDRAM Slice Card to the XP-SKC-U16 Slicekit Core board using the connector marked with the ``SQUARE``.
   #. Connect XA-SK-SCR480 Slice Card to the XP-SKC-U16 Slicekit Core board using the connector marked with the ``DIAMOND``.
...
++++++++++++++++++++++++++++++++
   ...
   #. Locate the ``Display Spectrum`` item in the xSOFTip pane on the bottom left of the window and drag it into the Project Explorer window in the xTIMEcomposer. This will also cause the modules on which this application depends to be imported as well. 
...
Now that the application has been compiled, the next step is to run it on the Slicekit Core Board using the tools to load the application over JTAG (via the XTAG2) into the xCORE multicore microcontroller.
   ...
   #. The spectrum of segments of mixed signal of two simulated chirp waveforms is displayed on LCD. 


Next Steps
++++++++++

   #. ``FFT_POINTS`` gives the number of signal samples taken for FFT computation. This decides the number of level meter bands which can be varied. ``FFT_SINE`` is also varied accordingly.
