...
*/
void image_write_line_nonblocking(chanend server, unsigned line,
		unsigned image_no, unsigned buffer[]) {
	unsigned buffer_pointer;
	server	<: (char)CMD_WRITE_LINE_NONBLOCKING;
...
*/
void image_read_line_nonblocking(chanend server, unsigned line,
		unsigned image_no, unsigned buffer[]) {
	unsigned buffer_pointer;
	server	<: (char)CMD_READ_LINE_NONBLOCKING;
	asm("mov %0, %1" : "=r"(buffer_pointer) : "r"(buffer));
	master {
		server <: image_no;
		server <: line;
...
*/
void image_read_partial_line_nonblocking(chanend server, unsigned line,
		unsigned image_no, unsigned buffer[], unsigned line_offset,
		unsigned word_count, unsigned buffer_offset) {
	unsigned buffer_pointer;
  if(word_count==0)
    return;
	server	<: (char)CMD_READ_PARTIAL_LINE_NONBLOCKING;
	asm("mov %0, %1" : "=r"(buffer_pointer) : "r"(buffer));
...
/*
  This sends SDRAM refresh commands to the SDRAM. It will send num_cycles of them.

  chanend server -  This is a channel to the lcd_sdram_manager.
  unsigned num_cycles - The count of refresh commands to send.
*/
