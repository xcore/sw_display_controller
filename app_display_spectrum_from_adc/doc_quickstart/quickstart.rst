...
To setup up the system:
   ...
   #. Set the ``XLINK`` to ``OFF`` on the sliceKIT Core board.
...
Various parameters defined in ``app_display_spectrum_from_adc.xc`` are listed below. Change them to see their effects. 
...
Change the colors of the level-meter display of spectrum given in ``level_meter_conf.h``.